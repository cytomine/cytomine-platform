Feature: [URS00001-TASK] Upload a task
  ...
  Scenario Outline: successful valid task upload
    ...
    Examples:
      | task   | task namespace                                 | task version | tar filepath | task uuid                            | storage identifier                            | image name                                           |
      | Task 1 | com.cytomine.app-engine.tasks.toy.add-integers | 0.1.0        | image.tar    | acde070d-8c4c-4f0d-9d8a-162843c10333 | task-acde070d-8c4c-4f0d-9d8a-162843c10333-def | com/cytomine/app-engine/tasks/toy/add_integers:0.1.0 |
  ...
  Scenario Outline: unsuccessful upload of duplicate task
    ...
    Examples:
      ...
      | Task 1 | com.cytomine.app-engine.tasks.toy.add-integers | 0.1.0        |

  Scenario Outline: unsuccessful upload of a task with invalid descriptor
    Given a "<task>" uniquely identified by an "<task namespace>" and a "<task version>"
    And this task is represented by a zip archive containing a task descriptor file "<task version>" and a docker image at "<docker image path>"
    ...
    Then App Engine fails to validate the "<task>" descriptor against the descriptor schema
    ...
    Examples:
      ...
      | Task 1 | com.cytomine.app-engine.tasks.toy.add_integers |              | image.tar         | image.tar                  | no version             |
      | Task 2 | com.cytomine.app-engine.tasks.toy.add_integers | 0.1.0        | image.tar         | docker/image.tar           | invalid image location |
      | Task 3 | com.cytomine.app-engine.tasks.toy.add_integers | 0.1.0        | myimage.tar       | image.tar                  | invalid image location |
