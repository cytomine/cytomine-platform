...
public class TaskValidationService {
    ...
    Logger logger = LoggerFactory.getLogger(TaskValidationService.class);

    @Autowired
    TaskRepository repository;

    public void checkIsNotDuplicate(UploadTaskArchive task) throws ValidationException {
        Task found = repository.findByNamespaceAndVersion(task.getDescriptorFileAsJson().get("namespace").textValue(), task.getDescriptorFileAsJson().get("version").textValue());
        if (found != null) {
    ...
    public void validateImage(UploadTaskArchive task) throws ValidationException, BundleArchiveException {
        checkManifestJsonExists(task);
    }
    private void checkManifestJsonExists(UploadTaskArchive task) throws BundleArchiveException, ValidationException {

        try {
            try (TarArchiveInputStream tarInputStream = new TarArchiveInputStream(new ByteArrayInputStream(task.getDockerImage()))) {
                ...
                if (manifestNotFound) {
                    logger.info("Validation error [manifest.json does not exist]");
    ...
    public void validateDescriptorFile(UploadTaskArchive task) throws ValidationException {
        ...
        Set<ValidationMessage> errors = getDescriptorJsonSchemaV7().validate(getDescriptorJsonNode(task));
    ...
    public static JsonNode getDescriptorJsonNode(UploadTaskArchive task) throws ValidationException {
